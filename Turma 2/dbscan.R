...
if (requireNamespace("fpc", quietly = TRUE) &&
  ...
  r
  boxplot(r,
          names = c('fpc', 'dbscan (dist)', 'dbscan (linear)',
                    'dbscan (kdtree)'),
          main = "Runtime comparison in ms")
  ## speedup of the kd-tree-based version compared to the fpc 
  #implementation
  median(t_fpc$time) / median(t_dbscan$time)
}else{print("Não tenho biblioteca")}
## End(Not run)

## Example 3: manually create a 
#frNN object for dbscan (dbscan only needs ids and eps)
nn <- structure(list(ids = list(c(2,3), c(1,3), c(1,2,3), c(3,5), 
                                c(4,5)), eps = 1),
                class = c("NN", "frNN"))
nn
dbscan(nn, minPts = 2)
